...
- 参照名: 全テーマ解放／表示名（日本語）: 全テーマを解放／説明: 全279テーマ・約2,500問が使い放題
...
```
社労士試験の「似ているけど違う」を横断テーマで整理。279テーマ・約2,500問の過去問○×を、現行法（令和7年度）で全問チェック済み。間違えた問題は「弱点補強」に自動で集約。まず3テーマを無料でお試しできます。
```
**説明文**
```
...
■ 特長
・横断279テーマ：資格の得喪／保険料の徴収／標準報酬／遺族の範囲／障害の要件 など、科目をまたいで比較表で整理
...
- 279テーマ・約2,500問の独自DBと学習機能（○×採点・弱点補強・苦手優先の復習）＝閲覧サイトではない
- IAPあり＝ネイティブ機能を使用
